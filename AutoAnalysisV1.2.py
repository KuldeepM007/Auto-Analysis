...
# Scan (safety measure) Contained
# Folder containing psr data can be copied directly
# Cycle and nchan can be changed per requirment below
# A PSRlist file must be provided, containing all PSRs being analysied 
...
PSRlistPath = f'{AnDir}/PSRlist.txt'
...
def copy(filename):
    ...
    if f'{Pulsar}' in RefFile:
        ...
        print(f'{filename} Not a listed pulsar.')
        global Counter
        Counter += 1
...
def Time_residual_plot(OutputPath,Pulsar):
    ...
    try:
        ...
        plt.errorbar(x,y,yerr = yerr, color='red', ls='', marker='o', capsize=5, capthick=1, markeredgecolor='black', ecolor='black')
        plt.xlabel("Time")
...
def analysis(files):
    for filename in files:
        ...
        n3cmd2 = f'singularity exec {SingImgPath} {DMcalcPath} -E {ParPath}/{Pulsar}.par -M {ModelPath}/{Pulsar}{MBand} -nch {nchanval} {TargetFilePath} >> {DMCalc_logsPath}'
...
ObsBands = ['BAND3.200','BAND5.200']

RefFile = np.loadtxt(PSRlistPath, dtype=str).T
for psr in RefFile:
    for band in ObsBands:
        cmd = f'mkdir -p {LandDir}/{obscycle}/{psr}/{band}/ ; touch {LandDir}/{obscycle}/{psr}/{band}/files_copied.txt'
        run(cmd, shell=True)
...
while True:
    ...
    if varst:
        ...
        print('Files copied :', len(varst) - Counter)
